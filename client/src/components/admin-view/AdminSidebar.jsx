...
const AdminSidebar = ({ isSidebarOpen, onClose }) => {
  ...
  return (
    <aside
      className={`lg:flex flex-col bg-black text-white w-52 fixed lg:static z-50 lg:z-auto transition-transform duration-300 ${
        isSidebarOpen ? "translate-x-0 h-screen" : "-translate-x-full"
      } lg:translate-x-0`}
    >
      <X
        className="float-end mr-2 mt-2 opacity-80 hover:cursor-pointer hover:opacity-100 lg:hidden"
        onClick={onClose}
      />
      <div
